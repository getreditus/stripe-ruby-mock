...
describe StripeMock do
  ...
  it "overrides stripe's request method" do
    StripeMock.start
    Stripe::APIRequestor.active_client.execute_request(:xtest, '/', api_key: 'abcde') # no error
    StripeMock.stop
  end

  it "overrides stripe's execute_request method in other threads" do
    StripeMock.start
    Thread.new { Stripe::APIRequestor.active_client.execute_request(:xtest, '/', api_key: 'abcde') }.join # no error
    StripeMock.stop
  end
  ...
  it "reverts overriding stripe's request method" do
    ...
    Stripe::APIRequestor.active_client.execute_request(:xtest, '/', api_key: 'abcde') # no error
    StripeMock.stop
    expect { Stripe::APIRequestor.active_client.execute_request(:x, '/', api_key: 'abcde') }.to raise_error Stripe::APIError
  end
  ...
  it "reverts overriding stripe's execute_request method in other threads" do
    ...
    Thread.new { Stripe::APIRequestor.active_client.execute_request(:xtest, '/', api_key: 'abcde') }.join # no error
    StripeMock.stop
    expect { Thread.new { Stripe::APIRequestor.active_client.execute_request(:x, '/', api_key: 'abcde') }.join }.to raise_error Stripe::APIError
  end
